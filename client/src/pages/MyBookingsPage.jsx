...
export default function MyBookingsPage({ socket, currentUser, embedMode = false }) {
  ...
  const handleCancelBooking = async (bookingId) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        alert(' Booking cancelled successfully. Slot freed. No money was charged.');
        fetchBookingsData();
      } else {
  ...
  const renderBookingCard = (b, isPast = false) => (
    <div key={b._id} className="card" style={{ opacity: isPast ? 0.75 : 1 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
        <span style={{
          background: isPast ? 'rgba(255,255,255,0.1)' : 'rgba(194, 210, 173, 0.15)',
      ...
      <h3 style={{ fontSize: '1.2rem', fontWeight: '600', color: '#fff' }}>{b.venueId?.name || 'Turf Venue'}</h3>
    ...
    <div className={embedMode ? "embedded-container" : "page-container"}>
      {!embedMode && (
        <div className="page-header">
          <h1 className="page-title"> My Bookings & Hosted Games</h1>
      ...
      {activeTab === 'bookings' && (
        <div>
          {loading ? (
            ...
            <div>
              {/* Upcoming Bookings */}
              <h4 style={{ fontSize: '1.1rem', fontWeight: '600', color: 'var(--pitch-green)', marginBottom: '1rem' }}>
